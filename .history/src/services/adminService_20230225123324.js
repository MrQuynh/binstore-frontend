...
export const getAConversationAdminService = async (id) => {
    try {
        const res = await request.get(`get-a-conversation-admin?conversationId=${id}`);
        return res;
    } catch (error) {
        console.log(error);
    }
};
export const sendTextAdminService = async (data) => {
    try {
        const res = await request.post(`create-a-message-admin`, data);
        return res;
    } catch (error) {
        console.log(error);
    }
