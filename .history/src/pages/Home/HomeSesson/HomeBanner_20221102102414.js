...
function HomeBanner() {
    const pagination = {
        ...
        renderBullet: function (index, className) {
            console.log(index);
            return '<span class={cx(' + className + ')}>' + (index + 1) + '</span>';
            // const text = `<p className={cx('text-content',${className})}>
            //         <div className={cx('text')}>${listSlider[e].title[0]}</div>
            //         <div className={cx('text')}>${listSlider[e].title[1]}</div>
            //     </p>`;
