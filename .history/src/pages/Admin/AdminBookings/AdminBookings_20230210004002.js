...
function AdminBookings() {
    ...
    const handleConfirm = (item, index) => {
        confirmBooking({ id: item.id, status: 'R2' }).then((res) => {
    ...
    const handleCancel = (item) => {
        cancelService({ id: item.id, status: 'R4' }).then((res) => {
            if (res && res.errCode === 0) {
                setChange(Math.random());
            }
        });
                            ...
                            <tbody key={index}>
                                <tr>
                                    ...
                                    <td className={cx('confirm')}>
                                        <p onClick={() => handleConfirm(item, index)}>Confirm</p>
                                        <p onClick={() => handleCancel(item, index)}>Cancel</p>
                                    </td>
                                </tr>
                            </tbody>
                        ))}
