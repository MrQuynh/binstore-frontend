...
function ModalBuy({ infoProduct, setIsModal }) {
    let cartList = JSON.parse(localStorage.getItem('CART_LISTS')).reverse();
    const value = cartList.reduce((a, b) => a + b.priceDown.split('.').join(''), 0);
    console.log(value);
    return (
        <Portal>
                        ...
                        <div className={cx('sale-code')}>
                            ...
                            <div className={cx('sale-right')}>
                                <div className={cx('price-total')}>
                                    <p>Tổng tiền:</p>
                                    <span>
                                        {/* {cartList && cartList.length > 0 ? cartList.reduce((a, b) => +a + b, '') : '0'}đ */}
                                    </span>
                                </div>
                                <div className={cx('price-total')}>
                                    <p>Giảm:</p>
