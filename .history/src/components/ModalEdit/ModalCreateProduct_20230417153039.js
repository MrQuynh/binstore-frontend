...
import { createBrandService, getAllCode } from '~/services/productService';
...
function ModalCreateProduct({ setIsModal, action, dataProduct, setProductEdit }) {
    ...
    const brandData = codeBrain && codeBrain.find((item) => item.keyMap === dataProduct.brain);
    const [createBrain, setCreateBrain] = useState(brandData.value);
    ...
    const handleCreate = () => {
        if (action === 'CREATE') {
            ...
            createBrandService({ value: createBrain });
        } else {
